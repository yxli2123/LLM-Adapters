...
def train(
        ...
        output_dir: str = "/mnt/t-qingru/exp_results/gms8k",
        ...
        path_to_model_zoo: str = '/mnt/t-qingru/yixiao_model_zoo',
        reduced_rank: int = 8,
        num_bits: int = 4,
        num_iter: int = 0,
    ...
    output_dir = os.path.join(output_dir, base_model.split('/')[-1],
                                   f"bit{num_bits}", f"iter{num_iter}", f"rank{reduced_rank}",
                                   f"lr_{learning_rate}", f"seed{seed}")
    ...
    if ckpt_path is None:
        ckpt_path = os.path.join(path_to_model_zoo, base_model.split('/')[-1],
                                 f"bit{num_bits}", f"iter{num_iter}", f"rank{reduced_rank}")
    ...
    tokenizer = AutoTokenizer.from_pretrained(base_model, use_auth_toekn=HF_TOKEN, trust_remote_code=True)
